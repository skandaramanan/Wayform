...
- A decision (``) explicitly says plans are exposed via MCP tools only, with **no REST endpoint until Phase 3** — so if the POST is meant to create/edit a plan, building a new REST route now would contradict that decision unless you're saying we've reached Phase 3.
