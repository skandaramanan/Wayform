...
Which of these (or something else) matches what you actually need? I don't want to plan a hardcoded shared credential — that's the exact pattern this repo tore out already (``, the WEBHOOK_SECRET post-mortem in `tenancy.ts`, the `mlk_` token purge).
