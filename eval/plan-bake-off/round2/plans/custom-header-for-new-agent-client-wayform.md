...
2. Do you actually want a static shared-secret header (reintroducing the pre-`b804915` pattern, which would disable that client's OAuth fallback per the recorded decision), or did you mean "wire it up" in the OAuth-only style every other client uses?
