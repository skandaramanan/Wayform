...
async function main() {
    ...
    switch (sub) {
        case "hook":
            if (rest[0])
                process.env.MEMORYLAYER_HOOK_CLIENT = rest[0];
            await runHook();
            return;
        case "stop-review":
            if (rest[0])
        ...
        case "space":
            ...
            console.error(`Unknown "space" subcommand "${rest[0]}". Use: wayform space create --space <name> --owner <owner> --repo <repo> --gateway <url>`);
            process.exit(1);
            return;
        case undefined:
            await runServer();
            return;
        default:
            console.error(`Unknown command "${sub}". Use: wayform [hook|stop-review|init|doctor|space] …`);
            process.exit(1);
    }
}
main().catch((err) => {
