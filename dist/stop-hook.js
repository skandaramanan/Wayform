 ...
 */
export function bumpStopCount(sessionId) {
    const file = path.join(os.tmpdir(), `wayform-stop-${sessionId.replace(/[^\w.-]/g, "_")}`);
    try {
        let count = 0;
        try {
 ...
 */
export function wroteContextThisTurn(transcriptPath) {
    try {
...
export async function runStopHook() {
    ...
    if (sessionId && bumpStopCount(sessionId) % FIRE_EVERY !== 0)
        emitNoop();
