...
|---|---|---|---|---|
| 2026-07-06 | Skanda | memorylayer | *(unrecorded — token in session scratchpad; hash it or re-mint)* | active |
...
### ChatGPT (custom connector — token in URL, No Auth)

ChatGPT's connector UI takes only a URL (its auth dropdown is No Auth / OAuth,
no header field), so the token rides in the path. Settings → Connectors → New:

- **Server URL:** `https://memorylayer-gateway.memory-layer.workers.dev/mcp/mlk_THEIR_TOKEN`
- **Authentication:** No Auth → check "I understand" → Create

⚠️ URL-borne tokens leak via history/logs/screenshots — mint a **ChatGPT-only**
token per member (Checklist B) so it's revocable without touching their
header-based (Claude/Cursor) token. If Create still errors *after* the URL has a
valid token, it's no longer auth — check ChatGPT's tool-shape expectations.
