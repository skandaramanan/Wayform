...
export async function runServer(): Promise<void> {
          ...
          content: [
            {
              type: "text",
              text:
                "write_context is unavailable on the local server for gateway-only " +
  ...
  console.error(
    `wayform MCP server ready (author=${cfg.author}, store=${
      cfg.repoPath || `gateway-only via ${cfg.gatewayUrl ?? "unset gateway"}`
    })`,
  );
}
