...
export async function runSpaceCreate(
  ...
  if (parsed.owner) {
    log(
      `Allowlist ${parsed.owner} (operator, must be signed in as a gateway operator): curl -X POST ${parsed.gatewayUrl}/mcp/admin/allowlist -H "authorization: Bearer $WAYFORM_TOKEN" -H "content-type: application/json" -d '{"add":["${parsed.owner}"]}'`,
    );
  } else {
